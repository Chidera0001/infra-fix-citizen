import { offlineStorage, type PendingReport, type SyncResult } from '@/utils/offlineStorage';
...
function validateIssueData(issueData: any): { isValid: boolean; error?: string } {
  // Check title length (10-100 characters)
  if (!issueData.title || issueData.title.length < 10 || issueData.title.length > 100) {
    return {
      isValid: false,
  ...
  if (!issueData.description || issueData.description.length < 20 || issueData.description.length > 1000) {
    return {
      isValid: false,
...
class SyncService {
      ...
      for (const report of reportsToSync) {
        try {
          ...
          const photos = report.photos.map((photoBlob, index) => {
            return new File([photoBlob], `photo_${index}.jpg`, { type: 'image/jpeg' });
          });
          ...
          if (!validation.isValid) {
            await offlineStorage.updateSyncStatus(report.id, 'failed', validation.error);
          ...
          if (report.userId === 'offline-user') {
            ...
            if (userId) {
              ...
              issueResult = await issuesApi.createIssue(mappedIssueData, userId, photos);
            } else {
              throw new Error('Offline report needs to be linked to a user before syncing');
            }
          } else {
            // Use existing API for authenticated users (including previously offline users)
            issueResult = await issuesApi.createIssue(mappedIssueData, report.userId, photos);
          }
  ...
  async retryFailedReports(): Promise<SyncResult[]> {
    ...
    const reportsToRetry = failedReports.filter(report => report.syncStatus === 'failed');
