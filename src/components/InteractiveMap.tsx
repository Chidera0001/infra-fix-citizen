...
const InteractiveMap = ({
	...
	useEffect(() => {
		...
		console.log("Adding markers and heat map...");
		...
		if (isAdmin) {
			// Add heat map for admin view (using circles)
		...
		console.log("Markers and heat map added successfully");
	}, [issues, isAdmin, selectedLocation, onIssueClick, mapInitialized]);

	const handleZoomIn = () => {
		if (mapInstance.current) {
			mapInstance.current.setZoom(
				(mapInstance.current.getZoom() || 12) + 1
			);
		}
	};
