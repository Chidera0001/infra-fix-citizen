...
const AdminDashboard = ({ onBack }: AdminDashboardProps) => {
  ...
  const categoryData = useMemo(() => {
    const categoryCounts = filteredIssues.reduce(
      (acc, issue) => {
  ...
  const monthlyData = useMemo(() => {
    const monthCounts = filteredIssues.reduce(
      (acc, issue) => {
  ...
  const urgencyData = useMemo(() => {
    const urgencyCounts = filteredIssues.reduce(
      (acc, issue) => {
        ...
        <Card className='mb-6'>
          ...
          <CardContent>
            <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
              ...
              {/* Export Button */}
              <div className='space-y-2'>
                <label className='text-sm font-medium text-black'>Export</label>
                <Dialog
                ...
                >
                  ...
                  <DialogContent className='sm:max-w-md'>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Choose your preferred export format. The data will be
                        filtered according to your current selections.
                      </DialogDescription>
                    </DialogHeader>
                    <div className='grid gap-4 py-4'>
                      <div className='space-y-3'>
                        ...
                        <Button
                          onClick={handleFormattedExport}
                          variant='outline'
                          className='w-full justify-start'
                          disabled={filteredIssues.length === 0}
                        >
                          <BarChart className='mr-2 h-4 w-4' />
                          Export Formatted Data (Better readability)
                        </Button>
                        <Button
                      ...
                      <div className='text-center text-sm text-gray-500'>
                        {filteredIssues.length > 0 ? (
                          <p>
                            Exporting {filteredIssues.length} issues with
                            current filters
                          </p>
                        ) : (
                          <p>
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='mt-1 text-sm text-red-600'>
                    {openIssues.length > 0 ? 'Needs attention' : 'All resolved'}
                  </p>
                </div>
                  ...
                  <div className='space-y-4'>
                    <div className='flex items-center justify-between'>
                      ...
                      <span className='text-xl font-normal text-blue-600'>
                        {filteredIssues.length > 0 ? '5.2 days' : 'N/A'}
                      </span>
                    </div>
                    <div className='flex items-center justify-between'>
                      <span className='text-sm font-medium'>
                      ...
                      <span className='text-xl font-normal text-green-600'>
                        {filteredIssues.length > 0 ? '4.7/5' : 'N/A'}
                      </span>
                    </div>
                    <div className='flex items-center justify-between'>
                      <span className='text-sm font-medium'>
                      ...
                      <span className='text-xl font-normal text-green-600'>
                        {filteredIssues.length > 0
                          ? `${Math.round(
                              (resolvedIssues.length / filteredIssues.length) *
                                100
                            )}%`
                          : 'N/A'}
                      </span>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <span className='text-xl font-normal text-blue-600'>
                        {filteredIssues.length > 0 ? '2.1 hours' : 'N/A'}
                      </span>
                    </div>
                  </div>
                </CardContent>
          ...
          <TabsContent value='issues' className='space-y-6'>
            <div className='grid grid-cols-1 gap-6 lg:grid-cols-3'>
              ...
              <Card className='lg:col-span-2'>
                ...
                <CardContent>
                  {filteredIssues.filter(issue => issue.urgency === 'high')
                    .length > 0 ? (
                    <div className='space-y-4'>
                      {filteredIssues
                    ...
                    <div className='py-8 text-center text-gray-500'>
                      No high priority issues found for selected filters
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <CardHeader>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <CardDescription>
                      Complete list of citizen reports with management options
                    </CardDescription>
                  </div>
                  <Dialog
                    open={exportDialogOpen}
