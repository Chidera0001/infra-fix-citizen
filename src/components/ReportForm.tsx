...
const ReportForm = ({ onBack }: ReportFormProps) => {
	...
	const getCurrentLocation = async () => {
		...
		try {
			...
			let errorDescription = "Please allow location access or select location from map.";
			...
			if (error instanceof GeolocationPositionError) {
				switch (error.code) {
					case GeolocationPositionError.PERMISSION_DENIED:
						...
						errorDescription = "Location access is blocked. Please follow these steps: 1) Click the lock icon in your browser's address bar 2) Set location to 'Allow' 3) Refresh the page and try again. Or use 'Select from Map' instead.";
						break;
					case GeolocationPositionError.POSITION_UNAVAILABLE:
						errorTitle = "Location unavailable";
						errorDescription = "Unable to determine your location. Please try again or select location from map.";
						break;
					case GeolocationPositionError.TIMEOUT:
						errorTitle = "Location timeout";
						errorDescription = "Location request timed out. Please try again or select location from map.";
						break;
				}
	...
	const handleLocationSelected = (coordinates: { lat: number; lng: number }, address: string) => {
		setFormData(prev => ({
			...prev,
